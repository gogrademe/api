...
func (s *Store) GetTerm(id int) (*model.Term, error) {
	var r model.Term
	return &r, s.db.Get(&r, "select * from term WHERE id=$1", id)
}
...
func (s *Store) InsertTerm(term *model.Term) error {
	...
	var err error
	term.ID, err = insert(s.db, stmt, term)
	return err
}

// Update --
func (s *Store) UpdateTerm(term *model.Term) error {
	stmt := Update("term").SetN("name", "school_year", "created_at", "updated_at").Eq("id").String()
	term.UpdateTime()
...
func (s *Store) DeleteTerm(id int) error {
	stmt := `DELETE FROM term WHERE id=$1`
